...
function compile() {
	...
	var script = fs.readFileSync("showroom/script.js", "utf8");

	var c = child.exec("lessc showroom/style.less " + currentPath + "/build/showroom.css", (err, stdout, stderr) => {
		...
		pages.forEach(function(page){
			...
			var html = fs.readFileSync("showroom/index.jst.html", "utf8");
			...
			out += '<link rel="apple-touch-icon" sizes="128x128" href="app-icon-cashier.png">\n'
			...
			var style = fs.readFileSync("build/showroom.css", "utf8");
			out += "<style>\n";
			out += style;
			out += "</style>\n";
			...
			fs.writeFileSync("showroom.html", out);
...
if (arg == "make") {
	...
	fs.watch(currentPath + '/showroom', (eventType, filename) => {
